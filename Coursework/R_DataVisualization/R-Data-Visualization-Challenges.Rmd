...
```{r import}
## YOUR CODE HERE
heart <- read.csv("data/heart.csv", encoding = "UTF-8")
```
...
B <- ggplot(heart, aes(x = sex, y = chol)) +
  geom_boxplot()

# 不動到 dataframe 並設定 factor 階層
B <- ggplot(heart) +
  geom_boxplot(aes(x = factor(sex, 
                              levels = c("0", "1"), 
...
```{r challenge_3}
## YOUR CODE HERE
C <- ggplot(heart, aes(X.U.FEFF.age, chol, color = factor(sex))) +
