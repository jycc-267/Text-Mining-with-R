...
summary(as.factor(as.character(gap$continent))) # difference between chr and factor
...
gg_box <- ggplot(data = gap, 
                 aes(x = continent, 
                     y = lifeExp,
                     fill = continent)) + # 依照dataframe的變數塗上不同的顏色，同時就會有label跑出來
  geom_boxplot() + # 若在此輸入fill = black ，就只是把盒鬚圖塗上黑色而已
...
?aov
aov_gap <- aov(gap$lifeExp ~ gap$continent) # 方程式表示: y ~ x
...
?lm
gap_lm1 <- lm(lifeExp ~ gdpPercap + continent, data = gap)
summary(gap_lm1)
# Coefficients Asia 不見是因為 Asia 是第一個level
gap <- gap %>%
  mutate(continent = factor(continent, levels = c("Asia", "Africa", "Americas", "Europe", "Oceania")))
