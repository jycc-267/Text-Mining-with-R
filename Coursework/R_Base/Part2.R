...
gap <- read.csv("data/gapminder-FiveYearData.csv", 
                ...
                header = TRUE, 
                # Convert character character tring columns to factor type?
                stringsAsFactors = F,
...
## The first six rows are displayed by default, shift the setting into 10 rows
head(gap, n = 10)
...
heart <- readr::read_csv("data/heart.csv")

##### 2. Load the sleep_VIM.csv file. Save it in a variable named sleep_VIM
sleep_VIM <- readr::read_csv("data/sleep_VIM.csv")
